...
**注意**：RT 本身也有有效期（离线会话约 30 天），定期刷新可保持其活跃。如果 RT 也失效了（刷新接口返回 401），需要用[短信登录](#1-通过短信登录获取明文凭据推荐)重新获取 AT/RT 并更新对应变量。
...
| `TOKEN_EXPIRED` | 该账号登录令牌已过期（签到大概率已失败） | 用[短信登录](#1-通过短信登录获取明文凭据推荐)重新获取该账号的 AT/RT → 更新对应变量 → 重新部署 |
...
令牌过期。如果配置了 RT 自动续期，脚本会在到期前自动刷新，正常不会出现此问题。若仍出现，说明 RT 也失效了（离线会话约 30 天未活跃），需用[短信登录](#1-通过短信登录获取明文凭据推荐)重新获取该账号的 AT/RT → 更新对应变量 → 重新部署。
