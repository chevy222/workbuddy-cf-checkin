...
**问：页面底部的 `version` 是什么？**
是本次部署的构建版本，格式 `日期:当天第几次改动`——例如 `20260912:1` 表示 2026 年 9 月 12 日的第 1 次改动。它就是 `worker.js` 顶部的 `BUILD_VERSION` 常量，**每次改完代码要手动把冒号后的序号 +1**（跨天则换成当天日期、序号从 1 重新开始）。配合自动部署时，提交推送后刷新页面看这一行有没有变，就能立刻确认新版本上线了。如果没变，先 `Ctrl+F5` 强刷一下排除浏览器缓存。
