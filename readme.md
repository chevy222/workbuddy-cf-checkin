...
- 只想手动调试某一个账号：URL 加 `&account=账号名`，例如 `?action=auto&account=主号`。
...
直接用浏览器打开 `https://<你的worker域名>/` 是首页（显示已配置账号与令牌剩余天数）；打开各 action 返回结果卡片页；`?action=log` 是日志表格页（**每 60 秒自动刷新**，点「详情」查看单条完整 JSON）。

| 页面 | 地址 |
|---|---|
| 首页 / 操作导航 | `/` |
| 日志列表（表格、自动刷新） | `/?action=log` |
| 第 N 条记录详情 | `/?action=log&i=0`（i 为列表序号，从 0 开始） |
| 执行某动作并看结果页 | `/?action=auto` 等 |

### 程序调用 = JSON（和旧版一致）

- `curl`、脚本等默认不带 `Accept: text/html`，返回的仍是 JSON；
- 浏览器里想强制看 JSON，URL 加 `&format=json`（或 `&raw=1`），例如 `/?action=log&format=json`。

### action 一览

| action | 作用 |
|---|---|
| `auto` | 每日自动化：签到 + 成长中心（Cron 跑的就是它；签到失败的账号会跳过成长中心，不白打请求） |
| `growth` | 仅成长中心（不签到） |
| `status` | 仅查签到状态（调试用） |
| `claim` | 仅领取签到（调试用，幂等） |
| `all` | 查状态 + 领取一起返回（调试用） |
| `log` | 运行记录页面 / JSON（需配置 KV，见下） |

可选 URL 参数：

| 参数 | 作用 |
|---|---|
| `account=账号名` | 多账号时只执行指定账号 |
| `key=xxx` | 配置了 `WORKER_SECRET` 时的访问密钥，页面内链接会自动透传 |
| `i=N` | 配合 `action=log` 查看第 N 条详情 |
| `format=json` / `raw=1` | 强制返回 JSON，不渲染页面 |

**注意**：必须显式带上 `?action=` 参数才会执行动作；直接访问根路径 `/` 只会返回首页/说明，不执行任何操作（扫描器或误访问不会触发签到流程，也不会产生日志）。动作结果页**不会**自动刷新，避免每 60 秒重复执行。
...
配置后 `?action=log` 才能看到最近 30 次运行记录：
...
请求头不带 `Accept: text/html`（curl 默认即是），或 URL 加 `&format=json`。
