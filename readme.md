...
| 执行某动作并看结果页 | `/auto`、`/growth`、`/status`、`/claim`、`/all` |

> 旧版 `/?action=auto`、`/?action=log&i=0` 等查询串写法仍然兼容，旧书签不会失效。

### 程序调用 = JSON

- `curl`、脚本等默认不带 `Accept: text/html`，返回的仍是 JSON；
- 浏览器里想强制看 JSON，URL 加 `?format=json`（或 `?raw=1`），例如 `/logs?format=json`。
...
| `/status` | `/status` | `status` | 仅查签到状态（调试用） |
| `claim` | 仅领取签到（调试用，幂等） |
| `/all` | `all` | 查状态 + 领取一起返回（调试用） |
...
**注意**：直接访问根路径 `/` 只会返回首页/说明，不执行任何操作（扫描器或误访问不会触发签到流程，也不会产生日志）；动作必须通过对应路径（或兼容的 `?action=`）触发。动作结果页**不会**自动刷新，避免每 60 秒重复执行。
